...
void MultiPassSimplifier::initializePasses() {
    ...
    passes.push_back({
        ...
        // Rule: x^a * x^b -> x^(a + b) (Combines like terms with the same base in multiplication)
        { [](const ExprNode& node) { return node.value == "*" && node.left->value == "^" && node.right->value == "^" &&
                                      node.left->left->value == node.right->left->value; },
          [](ExprNode* node) {
    ...
    passes.push_back({
        ...
        { [](const ExprNode& node) { return node.value == "+" && ((node.left->value == "^" && node.left->left->value == "sin" && node.right->left->value == "cos") ||
    ...
    passes.push_back({
        // Re-check for opportunities to fold constants after transformations
        { [](const ExprNode& node) { return node.type == OPERATOR && node.left->type == NUMBER && node.right->type == NUMBER; },
          [](ExprNode* node) { return foldConstants(node); } }
    });
}
